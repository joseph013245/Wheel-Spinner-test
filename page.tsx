...
function chooseRandom<T>(arr: T[]) { return arr[Math.floor(Math.random() * arr.length)]; }
...
export default function Page() {
  ...
  const handleStart = async () => {
    ...
    await runTransaction(stateRef, (curr: GameState | null) => {
      ...
      return { ...curr, started: true, currentSpinnerId: null, remainingPlayerIds, countries: curr.countries.slice(0, GAME_SIZE), results: [] };
    });
  };
  ...
  const wheelData = useMemo(() => (state?.countries || []).map(c => ({ option: `${c.flag} ${c.name}` })), [state?.countries]);
  ...
  const onStopSpinning = async () => {
    ...
    await runTransaction(stateRef, (curr: GameState | null) => {
      ...
      return { ...curr, countries: nextCountries, remainingPlayerIds: nextRemaining, currentSpinnerId: null,
        results: [...curr.results, { playerName, country: winningCountry.name, flag: winningCountry.flag }] };
    });
  };

  const currentSpinnerName = useMemo(() => state?.currentSpinnerId ? (players[state.currentSpinnerId]?.username || "(player)") : null, [state?.currentSpinnerId, players]);
    ...
    <main className="container space-y-6">
      ...
      {!me && (
        <section className="card space-y-3">
          ...
          <div className="flex gap-2">
            <input className="input" placeholder="Enter a username" value={usernameInput}
              onChange={(e) => setUsernameInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleJoin()} />
      ...
      <section className="card space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {Object.values(players).map(p => (<div key={p.id} className="px-3 py-1 rounded-full bg-gray-100 dark:bg-neutral-800">{p.username}</div>))}
      ...
      {state?.started && (
        <section className="card space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm opacity-70">Remaining: {state.countries.length} countries · {state.remainingPlayerIds.length} players</div>
          </div>
          <div className="flex items-center gap-2">
            <button className="btn btn-secondary" onClick={pickSpinner} disabled={!canPickSpinner}>Pick random player</button>
            {currentSpinnerName && (<div className="text-sm">Chosen to spin: <span className="font-semibold">{currentSpinnerName}</span></div>)}
          </div>
          <div className="flex flex-col items-center gap-3">
            <div className="w-full max-w-[360px]">
              <Wheel key={wheelData.map(d => d.option).join("|")} mustStartSpinning={wheelMustStart} prizeNumber={prizeNumber}
                data={wheelData} onStopSpinning={onStopSpinning} outerBorderColor={"#111"} radiusLineColor={"#333"} fontSize={14} />
            </div>
            <button className="btn btn-primary" onClick={spin} disabled={!amChosen || spinning || gameOver || (state?.countries.length ?? 0) === 0}>
              {amChosen ? (spinning ? "Spinning…" : "Spin the wheel") : "Waiting for spinner"}
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="table">
              <thead><tr><th className="w-1/2">Player</th><th className="w-1/2">Result</th></tr></thead>
              <tbody>{state.results.map((r, idx) => (<tr key={idx}><td>{r.playerName}</td><td>{r.flag} {r.country}</td></tr>))}</tbody>
            </table>
          </div>
          {gameOver && (<div className="text-center text-sm opacity-80">Game complete! Refresh to reset the room.</div>)}
        </section>
      )}
